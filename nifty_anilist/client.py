...
from nifty_anilist.utils.request_utils import run_request_with_retry, get_anilist_schema
...
class AnilistClient:
    ...
    def __init__(self, user_id: Optional[UserId] = None, use_auth: bool = True) -> None:
        self.gql_client = self._create_client(user_id, use_auth)
    ...
    def _create_client(self, user_id: Optional[UserId] = None, use_auth: bool = True) -> Client:
        ...
        return Client(transport=transport, schema=get_anilist_schema())
    ...
    async def anilist_request(self, query_request: GraphQLRequest) -> Dict[str, Any]:
